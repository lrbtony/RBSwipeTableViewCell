...
//        temBtn.tag = i;
        CGRect temRect = temBtn.frame;
        temRect.origin.x = SC_SCREEN_WIDTH - temRect.size.width ;
        temRect.origin.y = (self.cellHeight - 10) /  _rightBtnArr.count * i;
        temRect.size.height = (self.cellHeight - 10) /  _rightBtnArr.count;
        temBtn.frame = temRect;
...
//        }
    ...
    _rightfinalWidth = lastWidth;
//    if (!_rightfinalWidth || _rightfinalWidth < lastWidth) {
//        _rightfinalWidth = lastWidth;
//    }
...
- (void)setScrollView{
    ...
    _SCContentView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, SC_SCREEN_WIDTH, _cellHeight - 10)];
    _SCContentView.backgroundColor = [UIColor whiteColor];
//    int R = (arc4random() % 256) ;
//    int G = (arc4random() % 256) ;
//    int B = (arc4random() % 256) ;
//
//    [_SCContentView setBackgroundColor:[UIColor colorWithRed:R/255.0 green:G/255.0 blue:B/255.0 alpha:1]];
...
//    NSLog(@"translation----(%f)----loaction(%f)",translation.x,location.y);
    switch (recognizer.state) {
        ...
        case UIGestureRecognizerStateChanged:
            ...
            if (translation.x<0 && fabs(self.SCContentView.frame.origin.x) <= _rightfinalWidth) {
                if (fabs(self.SCContentView.frame.origin.x) == _rightfinalWidth) {
